...
class question_10_rate_wc extends question {

    public $type_name = "1-10 Rate w/o comments"; //loaded to database on install / update
    ...
    function display(&$mform, $form, $data, $order) {
        ...
        for ($i = 0; $i < self::max_rating; $i++) {
            $radioarray[] = &$mform->createElement('radio', "response[$order]", '', $i + 1, $i + 1);
        }
        ...
        $mform->addHelpButton("response_grp[$order]", 'question_10_rate', 'local_evaluations');
...
//        $mform->setType('text', PARAM_RAW);
    ...
    static function process_response_for_output($response, $comment) {
        $response = $response . get_string('question_10_rate_response', 'local_evaluations');
...
//        $output .= $comment;
    ...
    static function average($responses) {
        ...
        $output = get_string('average', 'local_evaluations') . ' ' . $average . '/' . self::max_rating;
    ...
    static function median($responses) {
        ...
        $output = get_string('median', 'local_evaluations') . ' ' . $median;
    ...
    static function mode($responses) {
        ...
        $output = get_string('mode', 'local_evaluations') . ' ' . $mode;
