...
class question_5_rate extends question {

    public $type_name = "1-5 Rate"; //loaded to database on install / update
    ...
    function display(&$mform, $form, $data, $order) {
        ...
        $abr = array(
            get_string('strongly_disagree_ab', 'local_evaluations'),
            get_string('disagree_ab', 'local_evaluations'),
            get_string('neutral_ab', 'local_evaluations'),
            get_string('agree_ab', 'local_evaluations'),
            get_string('strongly_agree_ab', 'local_evaluations'),
        );
        ...
        for ($i = 0; $i < self::max_rating; $i++) {
            $radioarray[] = &$mform->createElement('radio', "response[$order]", '', $abr[$i], $i + 1);
        }
    ...
    static function process_response_for_output($response, $comment) {
        ...
        $response = $response . get_string('question_5_rate_response', 'local_evaluations') . " : " . $verbous;



        $output = $response;
        $output .= '<br>';
        $output .= '<b>' . get_string('comments_c', 'local_evaluations') . '</b>';
        $output .= $comment;
    ...
    static function string_equiv($response) {
        ...
        switch ($response) {
            case 1:
                $response_string .= get_string('strongly_disagree', 'local_evaluations');
                break;
            case 2:
            ...
            case 4:
                ...
                break;
            case 5:
                $response_string .= get_string('strongly_agree', 'local_evaluations');
                break;
        }
