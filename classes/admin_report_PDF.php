...
class admin_report_PDF extends TCPDF {
...
// set font
        ...
        $output = get_string('start_header','local_evaluations') . ': '.date('F j Y @ G:i',$eval->start_time);      
        $this->Cell(0, $h = 0, $output, $border = 0, $ln = 1, $align = 'l', $fill = false, $link = '', $stretch = 0, $ignore_min_height = false, $calign = 'T', $valign = 'M');
        
        $output = get_string('end_header','local_evaluations') . ': '.date('F j Y @ G:i',$eval->end_time);      
        $this->Cell(0, $h = 0, $output, $border = 0, $ln = 1, $align = 'l', $fill = false, $link = '', $stretch = 0, $ignore_min_height = false, $calign = 'T', $valign = 'M');

        $duration_array = date_diff_format($eval->end_time,$eval->start_time);
        $output = get_string('duration','local_evaluations') . ': '. $duration_array;      
        $this->Cell(0, $h = 0, $output, $border = 0, $ln = 1, $align = 'l', $fill = false, $link = '', $stretch = 0, $ignore_min_height = false, $calign = 'T', $valign = 'M');
...
//randomize responses order
            ...
            $responses_data = array(); //The actual response data
            ...
            //Itterate responses for display               
            $response_i = 1;
            foreach ($responses as $response) {
    ...
    public function display_statistics($class, $responses_data) {
        ...
        if ($class::is_count_responses()) {
            ...
            $image_path = $class::count_responses($responses_data);
            $this->Image($image_path, '', '', '', '', 'png', '', 'N', true, 150, '', false, false, 0, false, false, false);
            $this->ln();
